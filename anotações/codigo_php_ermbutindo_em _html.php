<?php
 /*
  Existe algumas formas de fazer isso atraves de tags diferentes mas algumas tegs especificas serrvem somente para mostrar 
  valores de variaveis e não suiportam operações 
  
   tag padrão        <php? codigo aqui ?>   
   tag de impressão  <?= codigo aqui   ?> funciona 
   tag curta         <? código aqui    ?> não existe mais no ini da versão 8.2
   ASP tag           <% codigo aqui    %> parou de funcionar na versão 7

   comentários em php 

   // comentário de uma linha
   
   /*
      comentário para varias linhas 
   */

   # comentário para uma linha -> comentários desse tipo podem ser encontrados tanto em scripts php quanto em shell  



?>